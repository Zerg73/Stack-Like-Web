...
任务清单:
├── [ ] 1.1 安装 pixi.js 依赖
├── [ ] 1.2 创建 PixiApplication 封装
├── [ ] 1.3 创建渲染层接口 (GameRenderer)
├── [ ] 1.4 集成到 GameView
└── [ ] 1.5 验证基本渲染
```
...
任务清单:
├── [ ] 2.1 创建 CardSprite 类
├── [ ] 2.2 创建 SpritePool (精灵池，复用)
├── [ ] 2.3 实现卡牌绘制 (背景/名字栏/emoji)
├── [ ] 2.4 实现堆叠渲染 (多层叠加)
├── [ ] 2.5 状态同步 (选中/悬停/拖拽)
└── [ ] 2.6 性能测试 (100+ 卡牌)
```
...
```
任务清单:
├── [ ] 3.1 实现 ViewportController 类
├── [ ] 3.2 缩放 (以视口中心为锚点)
├── [ ] 3.3 平移 (边界限制)
├── [ ] 3.4 鼠标滚轮缩放
├── [ ] 3.5 键盘快捷键支持
└── [ ] 3.6 与 gameStore 状态同步
```
...
任务清单:
├── [ ] 4.1 拖拽事件系统
├── [ ] 4.2 拖拽开始 (pointerdown)
├── [ ] 4.3 拖拽移动 (pointermove)
├── [ ] 4.4 拖拽结束 (pointerup)
├── [ ] 4.5 碰撞检测 (与堆叠/槽位)
├── [ ] 4.6 分离/合并逻辑
└── [ ] 4.7 与 DragModule 集成
